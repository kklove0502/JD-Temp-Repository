...
def csv_to_df(i):
    ...
    df = pd.read_csv('D:/PythonProject/JD_FactorFactory01/DataDownload/min5/%s' % i, names=['Date','Open','High','Low','Close','Volume','OpenInterest'])
...
if __name__ == '__main__':
    ...
    df = handle_data_CR_factor('D:/PythonProject/JD_FactorFactory01/DataDownload/min5')

    factor = 'CR'
    beginDate = '20100101'
    endDate = '20121231'

    IC_sr = indicator_IC(df, factor=factor, beginDate=beginDate, endDate=endDate)
    print("====" * 20)
    print("在" + beginDate + "--"  + endDate + "时间内，" + factor + "因子在各个品种的IC值检测为：")
    print(IC_sr)
...
df_swap_cu = df_swap.loc['cu']

# rolling对象的corr方法默认使用pearson；
# 若使用df/sr，corr方法有method='pearson'参数可选；
kk = (df_swap_cu['CR'].rolling(5)).corr(df_swap_cu['NextReturn'].rolling(5))
df_swap_cu['CR_IC_5'] = kk

# 小验证
a = df_swap_cu['NextReturn'][1:6]
b = df_swap_cu['CR'][1:6]
m = a.cov(b) / (a.std() * b.std())

a.corr(b,method='pearson')
